...
Purpose: Give AI agents the minimal, actionable knowledge to build, run, and inspect the Grid Pulse Analytics microservices.
...
- Ingestion → Kafka → Usage → InfluxDB & Alerts → Alerting consumer
  - Topic `energy-usage`: produced by `ingestion-service` (`ingestion-service/src/main/java/com/glazowr/ingestion_service/service/IngestionService.java`) and consumed by `usage-service` (`usage-service/src/main/java/com/glazowr/usage_service/service/UsageService.java`).
  - Topic `energy-alerts`: produced by `usage-service` (aggregation/threshold logic) and consumed by `alert-service` (`alert-service/src/main/java/com/glazowr/alert_service/service/AlertService.java`).
- InfluxDB usage: `usage-service/src/main/java/com/glazowr/usage_service/config/InfluxDBConfig.java` and writes/queries in `UsageService.java`.
- MySQL: DB name `grid_pulse_analytics`, init in `docker/mysql/init.sql`; JDBC URLs appear in services' `src/main/resources/application.properties`. Keycloak uses a separate MySQL instance in Compose (`keycloak-db`).

API Gateway (routing & security)
- Gateway port **9000**; routes under `/api/v1/...` proxy to localhost backends with **circuit breakers** (see `api-gateway/src/main/java/com/glazowr/api_gateway/route/*.java`).
...
- Package names use underscores: e.g. `com.glazowr.ingestion_service` (see each service `HELP.md` which documents this change).
...
- `ingestion-service/src/main/java/com/glazowr/ingestion_service/controller/IngestionController.java`
- `ingestion-service/src/main/java/com/glazowr/ingestion_service/service/IngestionService.java`
- `usage-service/src/main/java/com/glazowr/usage_service/service/UsageService.java`
- `usage-service/src/main/java/com/glazowr/usage_service/config/InfluxDBConfig.java`
- `alert-service/src/main/java/com/glazowr/alert_service/service/AlertService.java`
- `api-gateway/src/main/java/com/glazowr/api_gateway/route/UserServiceRoutes.java` (and sibling route classes)
- `api-gateway/src/main/java/com/glazowr/api_gateway/config/SecurityConfig.java`
